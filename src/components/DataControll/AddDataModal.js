...
const AddDataModal = (props) => {
  ...
  async function onAddSubmitHandler(event) {
    ...
    const response = await getData(`/cityItems/${cityIdRef.current.value}/items/${idRef.current.value}`);
    if (response) {
      console.log("Duplicated CityID!");
      return;
    }

    writeData(`/cityItems/${cityIdRef.current.value}/items/${idRef.current.value}`, {
      ...
      mainImage1: mainImage1Ref.current.value,
      mainImage2: mainImage2Ref.current.value,
      mainImage3: mainImage3Ref.current.value,
      desc: descRef.current.value,
      role: role1Ref.current.value,
      existence: existenceRef.current.value,
      heritage: heritageRef.current.value,
    });
